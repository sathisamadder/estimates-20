...
export class DatabaseService {
  ...
  static async createProject(project: Omit<FirebaseProject, 'id' | 'createdAt' | 'updatedAt'>): Promise<string> {
    try {
  ...
  static async getProjects(userId?: string): Promise<FirebaseProject[]> {
    try {
      ...
      if (userId) {
        q = query(projectsCollection, where('userId', '==', userId), orderBy('updatedAt', 'desc'));
      }
  ...
  static async updateProject(id: string, updates: Partial<FirebaseProject>): Promise<void> {
    try {
      const docRef = doc(projectsCollection, id);
  ...
  static async createClient(client: Omit<FirebaseClient, 'id' | 'createdAt' | 'updatedAt'>): Promise<string> {
    try {
      const now = Timestamp.now();
  ...
  static async getClients(userId?: string): Promise<FirebaseClient[]> {
    try {
      ...
      if (userId) {
        q = query(clientsCollection, where('userId', '==', userId), orderBy('name', 'asc'));
      }
  ...
  static async updateClient(id: string, updates: Partial<FirebaseClient>): Promise<void> {
    try {
  ...
  static async syncLocalToFirebase(projects: any[], clients: any[]): Promise<void> {
    try {
      // Sync projects
